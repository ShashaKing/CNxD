...
def add_query_var_to_matrix(var_arr, potential_orig, n_varibles, varible_id):
    ...
    #potential_extend = np.full((2,2,2), LOG_ZERO)
...
def msg_root_to_leaf(topo_order, children, no_of_chlidren, potential_orig, msg_sent_prev):
    ...
    for cid in topo_order:
        ...
        #msg = np.zeros((2,2))
        #msg [:,:] = potential_orig[cid,:,0,:] + potential_orig[cid,:,1,:] 
...
def get_marginal(potential_orig, clique_id_var_asChild, pair_var):
    #clique_ids = clique_id_var_asChild[query_id+1:] # the clique ids that will be refered
    clique_ids = clique_id_var_asChild[pair_var] # the clique ids that will be refered
    #print 'clique_ids: ', clique_ids
...
class Clique:
    ...
    def set_parent(self, parent):
        self.parent = parent
        #self.seperator_p[0] = np.intersect1d(self.nodes[:-1], self.parent.nodes[:-1])
...
#        print '**************In Learn Sructure************'
        self.clique_list = []
        self.n_cliques = topo_order.shape[0]
        #self.clique_node_arr = np.zeros((self.n_cliques,2))
        #self.jt_order = np.zeros(self.n_cliques)
        ...
        for i in xrange(1, topo_order.shape[0]):
            ...
            #clique_id = i-1
            clique_id = i
            #new_clique = Clique(clique_id, np.array([child, parent]), log_cond_cpt[i].flatten())
            new_clique = Clique(clique_id, np.array([child, parent]), cond_prob[i])
            #self.clique_node_arr[clique_id] = np.array([child, parent])
    ...
    def clique_to_matrix(self):
        ...
        for clq in self.clique_list:
            ...
            #self.clique_parent[i] = clq.parent
...
def main_jt():
    ...
    '''
    # for test purpose. Could also read the evidence file 
